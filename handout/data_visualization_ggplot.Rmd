...
```{r scatter-challenge, answer=TRUE, eval = FALSE}
ggplot(data = surveys_complete, 
...
<!-- ADD SMOOTHER TO THE PLOT AND COMPARE BETWEEN LOCAL/GLOBAL AESTHETICS -->


> ### Challenge
>
> Inspect the `geom_point` help file to see what other aesthetics are available. 
> Assign a new aesthetic to another aspect of your plot. 
> What happened? Does the aesthetic change if you use a continuous variable 
...
> 
> ```{r average-weight-time-series, answer=TRUE, }
...
```{r install-gridextra, message = FALSE}
